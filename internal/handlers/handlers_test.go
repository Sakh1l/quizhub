...
func adminToken(t *testing.T, mux *http.ServeMux) string {
  ...
  if w.Code != http.StatusOK { t.Fatalf("auth status=%d body=%s", w.Code, w.Body.String()) }
...
func TestHealthAndAdminAuth(t *testing.T) {
  ...
  if tok := adminToken(t, mux); tok == "" { t.Fatal("expected token") }
}
...
func TestCreateRoomAndJoin(t *testing.T) {
  ...
  if w.Code != http.StatusCreated { t.Fatalf("add question status=%d body=%s", w.Code, w.Body.String()) }

  w = reqJSON(mux, http.MethodPost, "/api/room/create", nil, tok)
  if w.Code != http.StatusCreated { t.Fatalf("create room status=%d body=%s", w.Code, w.Body.String()) }
  var room map[string]string
  _ = json.Unmarshal(w.Body.Bytes(), &room)
  code := room["room_code"]
  if code == "" { t.Fatal("expected room_code") }

  w = reqJSON(mux, http.MethodPost, "/api/join", map[string]string{"nickname":"Alice","room_code":code}, "")
  if w.Code != http.StatusCreated { t.Fatalf("join status=%d body=%s", w.Code, w.Body.String()) }
}
