...
type Handler struct {
	...
	Hub      *ws.Hub
	AdminPIN string
	// Legacy exported fields kept for merge/backward compatibility.
	QuestionIDs []int
	TimeLimit   int
...
func (h *Handler) getQuestionIDs() []int {
	...
	if len(h.questionIDs) == 0 && len(h.QuestionIDs) > 0 {
...
func (h *Handler) getTimeLimit() int {
	h.stateMu.RLock()
	defer h.stateMu.RUnlock()
	if h.timeLimit == 0 && h.TimeLimit > 0 {
		return h.TimeLimit
	}
	return h.timeLimit
}
...
func (h *Handler) setAdminToken(token string) {
	...
	h.AdminTokens = map[string]bool{token: true}
	h.stateMu.Unlock()
}
